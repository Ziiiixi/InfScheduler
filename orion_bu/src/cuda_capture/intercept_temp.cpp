...
extern "C" {
    void *cublas_handle_dummy = (void*)cublasCreate_v2;
}
...
cudaError_t cudaMalloc(void** devPtr, size_t size) {
	...
	if (idx < *num_total_clients && idx >= 0) {
		...
		func_record new_record = {MALLOC_RECORD, new_func_data, func_indexes[idx]};
...
cudaError_t cudaFree(void* devPtr) {
	...
	if (idx < *num_total_clients && idx >= 0) {
		...
		func_record new_record = {FREE_RECORD, new_func_data, func_indexes[idx]};
...
cudaError_t cudaMemcpy(void* dst, const void* src, size_t count, enum cudaMemcpyKind kind) {
	...
	if (idx < *num_total_clients && idx >= 0) {
		...
		func_record new_record = {MEMCPY_RECORD, new_func_data, func_indexes[idx]};
...
cudaError_t cudaMemset(void* devPtr, int  value, size_t count ) {
	...
	if (idx < *num_total_clients && idx >= 0) {
		...
		func_record new_record = {MEMSET_RECORD, new_func_data, func_indexes[idx]};
...
cudaError_t cudaLaunchKernel ( const void* func, dim3 gridDim, dim3 blockDim, void** args, size_t sharedMem, cudaStream_t stream ) {
	...
	bool wait = false;
	// DEBUG_PRINT("idx is: %d, num_total_clients is %d\n", idx, *num_total_clients);
	// if it's worker thread that execute this, stack this function adr to the queue
	// if (idx < *num_total_clients) {
	if (idx < *num_total_clients && idx >= 0) {
		DEBUG_PRINT("[INTERCEPTER-CATCH]-[%d] Caught cudaKernel,\n", func_indexes[idx]);
		pthread_mutex_lock(mutexes[idx]);
		new_kernel_record = {func, gridDim, blockDim, args, sharedMem, stream, false, 0};
		union func_data new_func_data;
		new_func_data.krecord = new_kernel_record;
		func_record new_record = {KERNEL_RECORD, new_func_data, func_indexes[idx]};
		kqueues[idx]->push(new_record);
		func_indexes[idx] += 1;
		pthread_mutex_unlock(mutexes[idx]);
		block(idx, mutexes, kqueues);
